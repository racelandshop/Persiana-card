...
const arrayFilter = (
  ...
  maxSize: number
) => {if (!maxSize || maxSize > array.length) {maxSize = array.length}
  ...
  for (let i = 0; i < array.length && filteredArray.length < maxSize; i++) {
    ...
    if (meetsConditions) {filteredArray.push(array[i])}
  }
...
export const findEntities = (
  ...
  if (includeDomains?.length) {conditions.push((eid) => includeDomains!.includes(computeDomain(eid)))}
  if (entityFilter) {conditions.push((eid) => hass.states[eid] && entityFilter(hass.states[eid]))}
